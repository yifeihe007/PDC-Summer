...
__global__ void update_cu(float *layer, int layer_size, int pos, float energy)
{
    /* 1. Compute the absolute value of the distance between the
        impact position and the k-th position of the layer */
    int k = threadIdx.x;
...
int main(int argc, char *argv[])
{
    ...
    cudaMalloc((void **)&d_layer, layer_size * sizef);
    ...
    for (i = 0; i < num_storms; i++)
    {
        ...
        for (j = 0; j < storms[i].size; j++)
        {
            ...
            cudaMemcpy(d_layer, layer, layer_size * sizef, cudaMemcpyHostToDevice);

            dim3 blockDim(layer_size);
            dim3 gridDim(1);

            //@@ Launch the GPU Kernel here
            update_cu<<<gridDim, blockDim>>>(d_layer, layer_size, position,
                                             energy);
            ...
            cudaMemcpy(layer, d_layer, layer_size * sizef, cudaMemcpyDeviceToHost);
        }
