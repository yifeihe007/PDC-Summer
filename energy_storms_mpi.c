...
void update(float *layer, int layer_size, int k, int pos, float energy) {
  ...
  /* NOTE: Real world atenuation typically depends on the square of the
     distance. We use here a tailored equation that affects a much wider range
     of cells */
...
/* ANCILLARY FUNCTIONS: These are not called from the code section which is
 * measured, leave untouched */
  ...
  if (layer_size <= 35) {
    ...
    for (k = 0; k < layer_size; k++) {
      ...
      /* Compute the number of characters.
         This number is normalized, the maximum level is depicted with 60
         characters */
...
int main(int argc, char *argv[]) {
  ...
  float local_maximum[num_storms / size + 1];

  int local_positions[num_storms / size + 1];
  for (i = 0; i < (num_storms / size + 1); i++) {
  ...
  /* START: Do NOT optimize/parallelize the code of the main program above this
   * point */
  ...
  MPI_Request recv_request1[size - 1];

  MPI_Status status[size - 1];
  MPI_Status status1[size - 1];
  ...
  if (rank != 0) {
    ...
    MPI_Isend(&local_maximum, num_storms / size, MPI_FLOAT, 0, 0,
              MPI_COMM_WORLD, &recv_request[rank - 1]);
    MPI_Isend(&local_positions, num_storms / size, MPI_INT, 0, 0,
              MPI_COMM_WORLD, &recv_request1[rank - 1]);

  } else {
    for (int j = 1; j < size; j++) {
      MPI_Irecv(&maximum[j * num_storms / size], num_storms / size, MPI_FLOAT,
                j, 0, MPI_COMM_WORLD, &recv_request[j - 1]);
      MPI_Irecv(&positions[j * num_storms / size], num_storms / size, MPI_INT,
                j, 0, MPI_COMM_WORLD, &recv_request1[j - 1]);
      printf("rank: %d\n", rank);
      printf("size: %d\n", size);
      printf("buffer size : %d\n", num_storms / size);
    }
    for (i = 0; i < (num_storms / size + 1); i++) {
      maximum[i] = local_maximum[i];
      positions[i] = local_positions[i];
    }
    MPI_Waitall(size - 1, recv_request, status);
    MPI_Waitall(size - 1, recv_request1, status1);
  }
