...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < num_storms; i++) {
    ...
    float *private_layer =
        (float *)calloc((storms[i].size + 1) * layer_size, sizeof(float));
#pragma omp parallel for
    for (int j = 0; j < storms[i].size; j++) {
      ...
      for (int k = 0; k < layer_size; k++) {
        ...
        update(&private_layer[j * layer_size], layer_size, k, position, energy);
      }
    }
    for (int j = 0; j < storms[i].size; j++)
      for (int k = 0; k < layer_size; k++) {
        layer[k] += private_layer[j * layer_size + k];
      }
...
#pragma omp parallel for reduction(max : maximum[i], positions[i])
